...
DB schema is versioned with **Alembic via Flask-Migrate** (`backend/migrations/`). See [additional/LEARNING.md](additional/LEARNING.md).

### Database Migrations (Alembic / Flask-Migrate)

Schema changes are **never** applied by editing the DB directly or via `db.create_all()`. Every change goes through an Alembic migration file committed to `backend/migrations/versions/`.

Container startup (`docker_entrypoint.sh`) and manual startup (`python run.py`) both run `flask db upgrade` before the app starts — this brings the DB schema to the latest revision (idempotent, safe to run every start).

Day-to-day usage (from `backend/`, with venv active and `FLASK_APP=run.py`):

```powershell
# after changing/adding a model in app/models/__init__.py
...
Notes:
- `FLASK_APP=run.py` must be set (already set via `ENV FLASK_APP=run.py` in `backend/Dockerfile`; set it manually in your shell for local/manual runs).
- The very first migration (`baseline schema`) captures the schema as it existed before this workflow was introduced — it was generated against an empty database and the pre-existing dev/prod databases were marked up to date with `flask db stamp head` (no DDL was re-run against data that already had the tables).
- Always inspect the autogenerated migration — Alembic does not reliably detect every change (e.g. some column type tweaks, check constraints) and may need manual edits.
...
|--------|----------|-------------|
| POST | `/auth/login` | Login, returns JWT token. Rejects `pending`/`suspended` accounts with a clear message; generic "Invalid credentials" for unknown user or wrong password (no user-enumeration hint) |
| POST | `/auth/register` | Self-registration — creates a user with `status=pending`, `role=null` (no access until an admin approves). Rate-limited to 5 attempts/IP/hour (Redis sliding window, same pattern as `BruteForceTracker`) |
| GET | `/auth/users` | List active users (for assignment dropdowns) |

### User Management (`/users`, admin only)

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/users/` | List all users; filter by `?status=pending\|active\|suspended`, `?search=` |
| POST | `/users/` | Admin creates a user directly (`status=active` immediately, bypasses approval) |
| PATCH | `/users/<id>` | Approve pending (`status=active` + `role`), suspend/reactivate (`status`), or reassign `role` |
| POST | `/users/<id>/reset-password` | Force password reset. Omit `new_password` to auto-generate one — returned **once** in the response, never stored in plaintext |
| DELETE | `/users/<id>` | Delete a user (incidents assigned to them are unassigned first) |

All endpoints require role `admin` (same `require_role('admin')` pattern as `/audit`) and are audit-logged via `services/audit_service.py` (`user.create`, `user.approve`, `user.update`, `user.reset_password`, `user.delete`). Admins cannot suspend/delete their own account through this API.
...
Design principle: **admin configures/manages the platform**, **analyst investigates and responds
to incidents**. Every write endpoint enforces its role server-side via `@require_role(...)` (or
an inline check) — the frontend hides/disables controls to match, but the backend is the source
of truth and is safe to call directly (e.g. via curl) with an analyst token.
...
| Detection Rules — create/edit/delete/toggle | ✓ | ✗ |
| Settings — appearance, language, alert sound | ✓ | ✓ |
| Settings — integration credentials (Groq/AbuseIPDB/SMTP/Telegram) + connection tests | ✓ | ✗ |
| Settings — detection lab mode, thresholds, escalation policy | ✓ | ✗ |
| Audit log (`/audit`) | ✓ | ✗ |
...
New accounts do **not** get a role or dashboard access automatically — every self-registration
is gated by admin approval. The login page (`frontend/src/pages/Login.js`) has a "Register" link
that toggles to a self-registration form (username/email/password → `POST /auth/register`):

```
POST /auth/register  →  status=pending, role=null  (cannot log in)
        │
        ▼  admin approves in User Management (PATCH /users/<id>)
    status=active, role=admin|analyst  (can log in, role-based menu applies)
        │
        ▼  admin suspends (PATCH /users/<id> { status: "suspended" })
    status=suspended  (existing JWTs are rejected on the next request — see auth_middleware.py)
```

- `pending` / `suspended` accounts are rejected at `/auth/login` with a specific message (not
  the generic "Invalid credentials" used for wrong username/password).
- `auth_middleware.verify_token()` re-checks the account's live status on **every** authenticated
  request, so a token issued before a suspension stops working immediately rather than waiting
  for the JWT to expire.
- Demo `admin` / `analyst` seed accounts are created with `status=active` directly (they skip
  the approval flow — see `app/utils/seeder.py`).
